...
\begin{tikzpicture}[node distance=1.3cm]
  ...
  \draw[biglink] (fc.south) -- node[linklabel, right]{MAVLink (TELEM1, 921600)} (jet.north);
...
\begin{tikzpicture}[node distance=0.8cm]
  ...
  \draw[biglink] (fc.south) -- node[linklabel, right]{UART \texttt{TELEM1} @ 921600} (jetson.north);
...
\midrule
Mission Planner $\leftrightarrow$ FC & USB or \texttt{TELEM2}        & 921600            & MAVLink: params, mode, mission, monitoring \\
RadioMaster TX $\to$ RX $\to$ FC     & RC input pins                 & PPM or SBUS      & Stick channels, mode switch (LOITER/POSHOLD/GUIDED) \\
Jetson $\leftrightarrow$ FC          & \texttt{TELEM1} UART          & 921600           & MAVLink: state, setpoints, arming, missions \\
